...
export const fetchAmedasStations = async (): Promise<
  Record<string, AmedasStation>
> => {
...
export const mergeAmedasData = (
  ...
  return Object.entries(stations).map(([id, station]) => {
    ...
    const prefName =
      prefNumber != null ? prefectures[String(prefNumber)] : null;
...
export async function fetchAmedasPoint(id: string, time: string) {
  ...
  for (let i = 0; i < 12; i++) {
    ...
    const fileTime = `${candidate.getFullYear()}${String(candidate.getMonth() + 1).padStart(2, "0")}${String(candidate.getDate()).padStart(2, "0")}_${String(candidate.getHours()).padStart(2, "0")}`;

    const url = `https://www.jma.go.jp/bosai/amedas/data/point/${id}/${fileTime}.json`;
    console.log(url);
    ...
    const latest = Object.keys(data).sort().at(-1);
...
export async function findLatestAmedasTime(): Promise<string> {
  ...
  for (let i = 0; i < 144; i++) {
    ...
    candidate.setMinutes(candidate.getMinutes() - i * 10);

    const time = `${candidate.getFullYear()}${String(candidate.getMonth() + 1).padStart(2, "0")}${String(candidate.getDate()).padStart(2, "0")}${String(candidate.getHours()).padStart(2, "0")}${String(candidate.getMinutes()).padStart(2, "0")}00`;
  ...
  throw new Error("最新のアメダス時刻が見つかりません");
}

export async function fetchAmedasHistory(id: string, time: string) {
  const result: {
    time: string;
    temp: number | null;
  ...
  const promises = Array.from({ length: 25 }, (_, i) => {
    ...
    const fileTime = `${date.getFullYear()}${String(date.getMonth() + 1).padStart(2, "0")}${String(date.getDate()).padStart(2, "0")}_${String(date.getHours()).padStart(2, "0")}`;
    ...
    return fetch(
      ...
      .then(async (res) => {
        ...
        return res.json() as Promise<Record<string, AmedasObservation>>;
      })
      .catch(() => null);
  });
